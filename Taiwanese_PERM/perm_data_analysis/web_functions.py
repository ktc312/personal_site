...
def read_summarize_ny():
    ...
    for i in the_list:
        the_list = i.split(',')
        list_of_lists.append(the_list)
    ny_mean = ("$ {:,.2f}".format(float(list_of_lists[2][2])))
    ny_std = ("$ {:,.2f}".format(float(list_of_lists[3][2])))
    ny_median = ("$ {:,.2f}".format(float(list_of_lists[9][1])))
    ny_min = ("$ {:,.2f}".format(float(list_of_lists[4][2].split('.')[0])))
    ny_max = ("$ {:,.2f}".format(float(list_of_lists[8][2].split('.')[0])))
...
def read_top_ten_state():
    ...
    for i in final_list:
        ...
        for x in state_list:
            if i[0] == x[:2]:
                new_list_1.append(x[3:-1])
                new_list_1.append(i[1])
                new_list_1.append(i[2])
        out_list_1.append(new_list_1)
    for i in final_list:
        new_list_2 = list()
        for x in state_list:
            if i[0] == x[:2]:
                new_list_2.append(x[3:-1])
                new_list_2.append("{:,.0f}".format(float(i[1])))
                new_list_2.append(i[2])
        out_list_2.append(new_list_2)
...
def read_top_10_state_mean():
    ...
    for i in the_list:
        ...
        for x in state_list:
            if i.split(',')[0] == x[:2]:
                new_list.append(x[3:])
                new_list.append("$ {:,.2f}".format(float(i.split(',')[1])))
                new_list.append("$ {:,.2f}".format(float(i.split(',')[2])))
...
def read_top_median_state():
    ...
    for i in the_list:
        for x in state_list:
            if i.split(',')[0] == x[:2]:
                out_list.append(x[3:])
                out_list.append("{:,.0f}".format(float(i.split(',')[1])))
    return out_list
