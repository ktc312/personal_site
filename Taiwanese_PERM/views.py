...
import sys, calendar
import perm_data_analysis.web_functions as web_functions
...
def index(request):
    ...
    total_perm = '-99'
    approved_perm = '-99'
    top_10_com_count = [['Intel', '209', '2.32%'],
                        ['Microsoft', '137', '1.52%'],
                        ['Google', '119', '1.32%'],
                        ['Qualcomm', '115', '1.28%'],
                        ['Apple', '66', '0.73%'],
                        ['Super Micro Computer', '65', '0.72%'],
                        ['Foxconn', '64', '0.71%'],
                        ['Broadcom', '56', '0.62%'],
                        ['Formosa Plastics', '53', '0.59%'],
                        ['Oracle', '50', '0.55%']]

    top_10_state_count_1 = [['-99', '-99', '-99'],
                          ['-99', '-99', '-99'],
                          ['-99', '-99', '-99'],
                          ['-99', '-99', '-99'],
                          ['-99', '-99', '-99'],
    ...
    context = {'end_year': end_year, 'end_mon': end_mon, 'total_perm': total_perm, 'approved_perm': approved_perm,
               'top_10_com_count': top_10_com_count, 'top_10_state_count_1': top_10_state_count_1,
               'top_10_state_count_2': top_10_state_count_2, 'ny_perm': ny_perm,
    ...
    try:
        context['top_10_state_count_1'] = web_functions.read_top_ten_state()[0]
        context['top_10_state_count_2'] = web_functions.read_top_ten_state()[1]
    except:
        e = sys.exc_info()[0]
        print "<p>Error: %s</p>" % e
    ...
    return render(request, 'tw_perm/index.html', context)


def qa_page(request):
    return render(request, 'tw_perm/qa_page.html')


def working(request):
    return render(request, 'tw_perm/working.html')
